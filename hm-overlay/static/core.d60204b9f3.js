...
const PAGE_META = {
  ...
  registerTasks: { title: "注册任务", sub: "本地浏览器注册任务、运行进度与日志" },
...
const PAGE_HREF = { overview: "/admin", keys: "/admin/keys", accounts: "/admin/accounts", registerTasks: "/admin/register-tasks", usage: "/admin/usage", logs: "/admin/logs", models: "/admin/models", settings: "/admin/settings", guide: "/admin/guide" };
let _softNavToken = 0;
...
function pageFromPath(pathname) {
  ...
  if (p.endsWith("/register-tasks")) return "registerTasks";
...
async function softNavigate(name, opts) {
  if (name === "registerTasks") { window.location.assign("/admin/register-tasks"); return Promise.resolve(true); }
  opts = opts || {};
