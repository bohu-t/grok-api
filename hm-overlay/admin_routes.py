...
def _split_registration_domains(value: Any) -> list[str]:
    raw: list[str] = []
    if isinstance(value, str):
        raw.extend(re.split(r"[,;\s]+", value))
    elif isinstance(value, (list, tuple, set)):
        for item in value:
            raw.extend(re.split(r"[,;\s]+", str(item or "")))
...
class RegistrationConfigBody(BaseModel):
    ...
    domain: str | None = Field(
        default=None,
        max_length=2048,
        description="Active domain(s) for selected mail provider",
    )
    moemail_domain: str | None = Field(default=None, max_length=2048)
    moemail_domains: list[str] = Field(default_factory=list, max_length=200)
    temp_mail_domain: str | None = Field(default=None, max_length=2048)
...
def _registration_cfg_from_body(body: EmailRegistrationBody | RegistrationConfigBody) -> dict:
    ...
    cfg = {
        ...
        "moemail_domains": getattr(body, "moemail_domains", None),
    ...
    return _normalize_moemail_domains(cfg)
...
async def get_email_registration_config(
    ...
    require_admin(request, x_admin_token)
    cfg = _normalize_moemail_domains(get_registration_config(include_secrets=True))
    return {
        "ok": True,
        "config": cfg,
...
async def put_email_registration_config(
    ...
    try:
        cfg = set_registration_config(
            _normalize_moemail_domains(body.model_dump(exclude_none=False)),
            replace=False,
        )
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e)) from e
...
async def start_email_registration(
    ...
    resolved = _normalize_moemail_domains(resolve_registration_inputs(overrides))
    selected_domain = _pick_moemail_domain(resolved)
